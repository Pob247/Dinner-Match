...
interface MemberMealPreference {
  id: number;
  meal_id: number;
  member_id: number;
  preference: string;
...
interface SuggestionRequest {
  eatingTonight?: number[];
}

interface SuggestionResponse {
  meal: string;
  mealData: Meal;
  eatingTonight: string;
  reason: string;
  warnings: string[];
  tips: string;
}
...
const db: Database.Database = new Database('dinner.db');
...
if (mealCount.count === 0) {
  const preloadedMeals: PreloadedMeal[] = [
    // Pasta & Italian
    {
...
app.get('/api/family', (_req: Request, res: Response<FamilyMember[]>) => {
  const members = db.prepare('SELECT * FROM family_members ORDER BY name').all() as FamilyMember[];
  res.json(members);
});

// Add family member
app.post('/api/family', (req: Request<{}, {}, Partial<FamilyMember>>, res: Response): void => {
...
app.put('/api/family/:id', (req: Request<{ id: string }, {}, Partial<FamilyMember>>, res: Response): void => {
  const { name, likes, dislikes, dietary } = req.body;
  if (!name) {
    res.status(400).json({ error: 'Name is required' });
    return;
...
app.delete('/api/family/:id', (req: Request<{ id: string }>, res: Response) => {
  db.prepare('DELETE FROM family_members WHERE id=?').run(req.params.id);
  res.json({ success: true });
});

// Get all meals (with optional search)
app.get('/api/meals', (req: Request<{}, {}, {}, { search?: string; category?: string }>, res: Response<Meal[]>) => {
  ...
  query += ' ORDER BY name';
  const meals = db.prepare(query).all(...params) as Meal[];
  res.json(meals);
});

// Get single meal with full details
app.get('/api/meals/:id', (req: Request<{ id: string }>, res: Response): void => {
  const meal = db.prepare('SELECT * FROM meals WHERE id = ?').get(req.params.id) as Meal | undefined;
  if (meal) {
    res.json(meal);
  } else {
...
// Add meal
app.post('/api/meals', (req: Request<{}, {}, Partial<Meal>>, res: Response): void => {
...
// Update meal
app.put('/api/meals/:id', (req: Request<{ id: string }, {}, Partial<Meal>>, res: Response): void => {
...
app.put('/api/meals/:id/favourite', (req: Request<{ id: string }>, res: Response): void => {
  const meal = db.prepare('SELECT is_family_favourite FROM meals WHERE id = ?').get(req.params.id) as Meal | undefined;
  if (!meal) {
    res.status(404).json({ error: 'Meal not found' });
    return;
  }
...
// Delete meal
app.delete('/api/meals/:id', (req: Request<{ id: string }>, res: Response) => {
...
app.get('/api/meals/:id/votes', (req: Request<{ id: string }>, res: Response) => {
  const votes = db.prepare(`
    SELECT mv.*, fm.name as member_name
    FROM meal_votes mv
    JOIN family_members fm ON mv.member_id = fm.id
    WHERE mv.meal_id = ?
  `).all(req.params.id) as MealVote[];
  const total = votes.reduce((sum: number, v: MealVote) => sum + v.vote, 0);
  res.json({ votes, total, count: votes.length });
});

// Vote for a meal
app.post('/api/meals/:id/vote', (req: Request<{ id: string }, {}, VoteRequest>, res: Response): void => {
...
// Remove vote
app.delete('/api/meals/:id/vote/:member_id', (req: Request<{ id: string; member_id: string }>, res: Response) => {
...
app.get('/api/meals/:id/preferences', (req: Request<{ id: string }>, res: Response<MemberMealPreference[]>) => {
  const prefs = db.prepare(`
    SELECT mmp.*, fm.name as member_name
    FROM member_meal_preferences mmp
    JOIN family_members fm ON mmp.member_id = fm.id
    WHERE mmp.meal_id = ?
  `).all(req.params.id) as MemberMealPreference[];
  res.json(prefs);
});

// Set preference for a meal (like or dislike)
app.post('/api/meals/:id/preference', (req: Request<{ id: string }, {}, PreferenceRequest>, res: Response): void => {
...
// Remove preference
app.delete('/api/meals/:id/preference/:member_id', (req: Request<{ id: string; member_id: string }>, res: Response) => {
...
app.get('/api/family/:id/preferences', (req: Request<{ id: string }>, res: Response<MemberMealPreference[]>) => {
  const prefs = db.prepare(`
...
app.get('/api/shopping-lists', (_req: Request, res: Response<ShoppingList[]>) => {
  const lists = db.prepare('SELECT * FROM shopping_lists ORDER BY created_at DESC').all() as ShoppingList[];
  res.json(lists);
});

// Create shopping list from selected meals
app.post('/api/shopping-lists', (req: Request<{}, {}, { name?: string; meal_ids: number[]; created_by?: string }>, res: Response): void => {
  ...
  const meals = db.prepare(`SELECT name, ingredients FROM meals WHERE id IN (${placeholders})`).all(...meal_ids) as Pick<Meal, 'name' | 'ingredients'>[];

  // Combine all ingredients
  let allIngredients: string[] = [];
  meals.forEach((meal: Pick<Meal, 'name' | 'ingredients'>) => {
    if (meal.ingredients) {
  ...
  const itemsJson = JSON.stringify(allIngredients.map((item: string) => ({ text: item, checked: false })));
  ...
  res.json({ id: result.lastInsertRowid, name, items: allIngredients, meals: meals.map((m: Pick<Meal, 'name'>) => m.name) });
});

// Update shopping list (mark items as checked)
app.put('/api/shopping-lists/:id', (req: Request<{ id: string }, {}, { items: ShoppingItem[] }>, res: Response): void => {
...
app.delete('/api/shopping-lists/:id', (req: Request<{ id: string }>, res: Response) => {
  db.prepare('DELETE FROM shopping_lists WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

// Get messages (last 50)
app.get('/api/messages', (_req: Request, res: Response<Message[]>) => {
  const messages = db.prepare('SELECT * FROM messages ORDER BY created_at DESC LIMIT 50').all() as Message[];
  res.json(messages.reverse());
});

// Send message
app.post('/api/messages', (req: Request<{}, {}, { sender: string; message: string }>, res: Response): void => {
...
app.post('/api/suggest', (req: Request<{}, {}, SuggestionRequest>, res: Response<SuggestionResponse | { error: string }>): void => {
  const { eatingTonight } = req.body;

  const allMembers = db.prepare('SELECT * FROM family_members').all() as FamilyMember[];
  ...
  const eatingMembers = eatingTonight && eatingTonight.length > 0
    ? allMembers.filter((m: FamilyMember) => eatingTonight.includes(m.id))
    : allMembers;
  ...
  // Score each meal
  const scored: ScoredMeal[] = meals.map((meal: Meal) => {
    let score = 50;
    const reasons: string[] = [];
    const warnings: string[] = [];
    const mealLower = (meal.name + ' ' + (meal.description || '') + ' ' + (meal.ingredients || '')).toLowerCase();

    eatingMembers.forEach((member: FamilyMember) => {
      if (member.likes) {
        member.likes.toLowerCase().split(',').map((l: string) => l.trim()).filter(Boolean).forEach((like: string) => {
      ...
      if (member.dislikes) {
        member.dislikes.toLowerCase().split(',').map((d: string) => d.trim()).filter(Boolean).forEach((dislike: string) => {
  ...
  scored.sort((a: ScoredMeal, b: ScoredMeal) => b.score - a.score);
  const best = scored[0];

  const eatingNames = eatingMembers.map((m: FamilyMember) => m.name).join(', ');
